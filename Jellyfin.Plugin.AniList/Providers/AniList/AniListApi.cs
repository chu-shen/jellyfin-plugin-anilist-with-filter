    ...
    characters(sort: [ROLE]) {
      edges {
        ...
        public async Task<RootObject> WebRequestAPI(string link)
        {
            ...
            File.WriteAllLines(Path.Combine("C:/SoftWare/Jellyfin/Data/cache/anilist","rateLimit.txt"), System.DateTime.Now+"：before wait...........");
            await RequestLimiter.Tick().ConfigureAwait(false);

            File.WriteAllLines(Path.Combine("C:/SoftWare/Jellyfin/Data/cache/anilist","rateLimit.txt"), System.DateTime.Now+"：delay wait...........");
            await Task.Delay(Plugin.Instance.Configuration.AniDbRateLimit).ConfigureAwait(false);

            File.WriteAllLines(Path.Combine("C:/SoftWare/Jellyfin/Data/cache/anilist","rateLimit.txt"), System.DateTime.Now+"：after wait...........");
